...
describe("JWT Session",()=>{
    it("Is logged in through local storage",()=>{
        ...
        cy.readFile(Cypress.config('fileServerFolder') + "/cypress/downloads/order-invoice_tarikbozdemir2602.csv")
        .then((text) => {
          return neatCSV(text);
        })
        .then((csv) => {
          cy.log(JSON.stringify(csv));
          console.log(csv);
          let productNameCSV=csv[0]["Product Name"]
          expect(realProductName).to.equal(productNameCSV)
          let orderNumberCSV=csv[0]["Invoice Number"]
          expect(realOrderNumber).to.equal(orderNumberCSV)
        });
